...
<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Prim Kuralı Düzenle</h1>
            <p class="text-gray-600">Mevcut prim kuralını düzenleyin</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form action="/commissions/rules/update/<?= $rule['id'] ?>" method="POST" id="ruleForm">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="branch_id" id="branch_id" required 
                            ...
                            <?php foreach ($branches as $branch): ?>
                                <option value="<?= $branch['id'] ?>" <?= ($rule['branch_id'] == $branch['id']) ? 'selected' : '' ?>>
                                    <?= esc($branch['name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <!-- Personel Seçimi -->
                <div id="userSelection" style="display: <?= ($rule['rule_type'] === 'user_specific') ? 'block' : 'none' ?>;">
                    ...
                    <select name="user_id" id="user_id" 
                        ...
                        <?php foreach ($users as $user): ?>
                            <option value="<?= $user['id'] ?>" <?= ($rule['user_id'] == $user['id']) ? 'selected' : '' ?>>
                ...
                <!-- Hizmet Seçimi -->
                <div id="serviceSelection" style="display: <?= (in_array($rule['rule_type'], ['service_specific', 'user_specific'])) ? 'block' : 'none' ?>;">
                    ...
                    <select name="service_id" id="service_id" 
                        ...
                        <?php foreach ($services as $service): ?>
                            <option value="<?= $service['id'] ?>" <?= ($rule['service_id'] == $service['id']) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select name="commission_type" id="commission_type" required 
                        ...
                        <?php foreach ($commissionTypes as $key => $label): ?>
                            <option value="<?= $key ?>" <?= ($rule['commission_type'] == $key) ? 'selected' : '' ?>>
                                <?= esc($label) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="commission_value" id="commission_value" 
                               ...
                               value="<?= esc($rule['commission_value']) ?>"
                               class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <p class="text-xs text-gray-500 mt-1" id="commissionHelp">
                        <?= ($rule['commission_type'] === 'percentage') ? 'Yüzdesel prim için 0-100 arası değer giriniz' : 'Sabit tutar için TL cinsinden değer giriniz' ?>
                    </p>
                </div>
                ...
                <div>
                    ...
                    <select name="is_package_rule" id="is_package_rule" required 
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="0" <?= ($rule['is_package_rule'] == 0) ? 'selected' : '' ?>>Normal Hizmetler</option>
                        <option value="1" <?= ($rule['is_package_rule'] == 1) ? 'selected' : '' ?>>Paketli Hizmetler</option>
                    </select>
                    <p class="text-xs text-gray-500 mt-1">
                ...
                <div>
                    ...
                    <select name="is_active" id="is_active" required 
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="1" <?= ($rule['is_active'] == 1) ? 'selected' : '' ?>>Aktif</option>
                        <option value="0" <?= ($rule['is_active'] == 0) ? 'selected' : '' ?>>Pasif</option>
                    </select>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBranchData() {
        const branchId = branchSelect ? branchSelect.value : <?= $rule['branch_id'] ?>;
            ...
            fetch(`/commissions/users-by-branch/${branchId}`)
                ...
                .then(users => {
                    ...
                    users.forEach(user => {
                        ...
                        userSelect.innerHTML += `<option value="${user.id}" ${selected}>${user.first_name} ${user.last_name} (${user.role_name})</option>`;
                    });
                });
            ...
            fetch(`/commissions/services-by-branch/${branchId}`)
                ...
                .then(services => {
                    ...
                    services.forEach(service => {
                        ...
                        serviceSelect.innerHTML += `<option value="${service.id}" ${selected}>${service.name} (₺${parseFloat(service.price).toFixed(2)})</option>`;
                    });
                });
        }
    }

...
<div class="p-6">
    ...
    <div class="flex items-center mb-6">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Şube Düzenle</h1>
            <p class="text-gray-600 mt-2"><?= esc($branch['name']) ?> şubesini düzenleyin</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="address" 
                              ...
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Şube adresi..."><?= old('address', $branch['address']) ?></textarea>
                </div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            if (old('working_hours')) {
                                $workingHours = json_decode(old('working_hours'), true) ?: [];
                            } elseif ($branch['working_hours']) {
                                if (is_string($branch['working_hours'])) {
                                    $workingHours = json_decode($branch['working_hours'], true) ?: [];
                                } elseif (is_array($branch['working_hours'])) {
                                    $workingHours = $branch['working_hours'];
                                } elseif (is_object($branch['working_hours'])) {
                                    // stdClass objesini array'e çevir
                                    $workingHours = json_decode(json_encode($branch['working_hours']), true) ?: [];
                                }
                            }
                            ?>
                            ...
                            <div class="flex items-center space-x-4 p-4 bg-white rounded-lg border border-gray-200">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox"
                                           ...
                                           value="1"
                                           <?= isset($workingHours[$dayKey]) && !empty($workingHours[$dayKey]['is_working']) ? 'checked' : '' ?>
                                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded day-toggle"
                                           data-day="<?= $dayKey ?>">
                                    <label for="<?= $dayKey ?>_working" class="ml-2 text-sm text-gray-600">
                                        Açık
                                    </label>
                                </div>
                                
                                <!-- Saat Seçimi -->
                                <div class="flex items-center space-x-2 day-hours" id="hours_<?= $dayKey ?>" style="<?= isset($workingHours[$dayKey]) && !empty($workingHours[$dayKey]['is_working']) ? '' : 'display: none;' ?>">
                                    <div class="flex items-center space-x-1">
                                        <label class="text-sm text-gray-600">Açılış:</label>
                                        <select name="<?= $dayKey ?>_start"
                                            ...
                                            <?php for ($hour = 6; $hour <= 23; $hour++): ?>
                                                <?php for ($minute = 0; $minute < 60; $minute += 30): ?>
                                                    ...
                                                    $selected = '';
                                                    if (isset($workingHours[$dayKey]['start_time']) && $workingHours[$dayKey]['start_time'] === $timeValue) {
                                                        $selected = 'selected';
                                                    } elseif (!isset($workingHours[$dayKey]) && $timeValue === '09:00') {
                                                        $selected = 'selected';
                                                    }
                                                    ?>
                                        ...
                                        <select name="<?= $dayKey ?>_end"
                                            ...
                                            <?php for ($hour = 6; $hour <= 23; $hour++): ?>
                                                <?php for ($minute = 0; $minute < 60; $minute += 30): ?>
                                                    ...
                                                    $timeValue = sprintf('%02d:%02d', $hour, $minute);
                                                    $selected = '';
                                                    if (isset($workingHours[$dayKey]['end_time']) && $workingHours[$dayKey]['end_time'] === $timeValue) {
                                                        $selected = 'selected';
                                                    } elseif (!isset($workingHours[$dayKey]) && $timeValue === '18:00') {
                                                        $selected = 'selected';
                                                    }
                                                    ?>
                                                    <option value="<?= $timeValue ?>" <?= $selected ?>><?= $timeValue ?></option>
                        ...
                        <input type="hidden" id="working_hours" name="working_hours" value="<?= old('working_hours', $branch['working_hours'] ? (is_string($branch['working_hours']) ? $branch['working_hours'] : json_encode($branch['working_hours'])) : '') ?>">
                    </div>
                </div>
    ...
    <div class="mt-6 bg-gray-50 rounded-lg p-4">
        <h4 class="text-sm font-medium text-gray-700 mb-2">Şube Detayları</h4>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
                <span class="text-gray-500">Oluşturulma:</span>
                <span class="text-gray-900"><?= date('d.m.Y H:i', strtotime($branch['created_at'])) ?></span>
            </div>
            <?php if ($branch['updated_at']): ?>
                <div>
                    <span class="text-gray-500">Son Güncelleme:</span>
                    <span class="text-gray-900"><?= date('d.m.Y H:i', strtotime($branch['updated_at'])) ?></span>
                </div>
            <?php endif; ?>
            <div>

...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Yeni paket tanımı oluşturun</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form action="/admin/packages/create" method="POST" class="p-6">
                            ...
                            <?php if ($userRole === 'admin') : ?>
                                <select name="branch_id" id="branch_id" required class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    <?php foreach ($branches as $branch): ?>
                                        <option value="<?= $branch['id'] ?>" <?= (isset($formData['branch_id']) && $formData['branch_id'] == $branch['id']) ? 'selected' : '' ?>>
                                            <?= esc($branch['name']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center">
                                    <input type="radio" name="type" value="session" 
                                           <?= (isset($formData['type']) && $formData['type'] === 'session') || !isset($formData['type']) ? 'checked' : '' ?>
                                ...
                                <label class="flex items-center">
                                    <input type="radio" name="type" value="time" 
                                           <?= (isset($formData['type']) && $formData['type'] === 'time') ? 'checked' : '' ?>
                                           class="mr-2" onchange="togglePackageType()">
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="validity_months" id="validity_months" min="1" required
                                   value="<?= esc($formData['validity_months'] ?? '6') ?>"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="6">
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" 
                                       <?= (isset($formData['is_active']) && $formData['is_active']) || !isset($formData['is_active']) ? 'checked' : '' ?>
                                       class="mr-2">
                                ...
                                foreach ($services as $service): 
                                    ...
                                    <label class="flex items-center mb-1">
                                        <input type="checkbox" name="service_ids[]" value="<?= $service['id'] ?>"
                                               <?= (isset($formData['service_ids']) && in_array($service['id'], $formData['service_ids'])) ? 'checked' : '' ?>
                                               class="mr-2">

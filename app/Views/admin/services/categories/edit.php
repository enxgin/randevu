...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900">Hizmet Kategorisi Düzenle</h1>
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Kategori Bilgilerini Düzenle</h3>
        </div>
        
        <form action="<?= base_url('admin/service-categories/edit/' . $category['id']) ?>" method="post" class="p-6 space-y-6">
            <?= csrf_field() ?>
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       value="<?= old('name', $formData['name'] ?? $category['name']) ?>" 
                       required
            ...
            <div>
                ...
                <?php if ($userRole === 'admin') : ?>
                    <select id="branch_id"
                            name="branch_id"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="" <?= (old('branch_id', $formData['branch_id'] ?? $category['branch_id']) == '') ? 'selected' : '' ?>>Genel Kategori (Tüm Şubeler İçin)</option>
                        <?php if (!empty($branches)) : ?>
                            <?php foreach ($branches as $branch) : ?>
                                <option value="<?= $branch['id'] ?>" <?= (old('branch_id', $formData['branch_id'] ?? $category['branch_id']) == $branch['id']) ? 'selected' : '' ?>>
                                    <?= esc($branch['name']) ?>
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                ...
                <?php else : ?>
                    ...
                    <div class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-md text-gray-700">
                        <?php if ($category['branch_id']) : ?>
                            <?= esc($branches[0]['name'] ?? 'Şube Bulunamadı') ?>
                        <?php else : ?>
                            Genel Kategori (Tüm Şubeler İçin)
                        <?php endif; ?>
                    </div>
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"><?= old('description', $formData['description'] ?? $category['description']) ?></textarea>
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" 
                       ...
                       name="is_active" 
                       value="1" 
                       <?= (old('is_active', $formData['is_active'] ?? $category['is_active']) == 1) ? 'checked' : '' ?>
